...
bool checkCache(int set, int setSizeExp,  vector<vector<int> > &cache, int tag, int counter)
{
...
int main() {
    ...
    while (!newfile.eof())
    {
        ...
        if (checkCache(set, setSizeExp, cache, tag, counter))
        {
            numhits++;
        }
...
float hitrate = (float) numhits/(float) counter;
    cout << "Simulation Results for Robert Meekins: " <<  endl << "\n";
    cout << "Hits: " << numhits << endl << "Accesses: " << counter << endl << "Hit rate: " << hitrate << endl;
